/*
 * bb_replies.h - reply templates and choice helpers, ported from
 * boombot/utils/replies.py.
 */
#ifndef BB_REPLIES_H
#define BB_REPLIES_H
...
inline const std::vector<std::string> SASSY_REPLIES_HIGH = {
...
inline const std::vector<std::string> SASSY_REPLIES_INVALID = {
...
inline const std::vector<std::string> SASSY_REPLIES_LOW = {
...
inline const std::vector<std::string> QUESTION_REPLY_VARIATIONS = {
    "I give {subject} {count_str} \xF0\x9F\x92\xA5",
    "{subject} definitely deserves {count_str} \xF0\x9F\x92\xA5",
    "My official rating for {subject}: {count_str} \xF0\x9F\x92\xA5",
    "Let's go with {count_str} \xF0\x9F\x92\xA5 for {subject}.",
    "Hmm, I'd say {subject} gets {count_str} \xF0\x9F\x92\xA5",
    "Solid {count_str} \xF0\x9F\x92\xA5 for {subject} from me.",
    "The boom-o-meter says: {count_str} \xF0\x9F\x92\xA5 for {subject}",
    "Without a doubt, {subject} gets {count_str} \xF0\x9F\x92\xA5",
};

inline const std::vector<std::string> PREVIOUSLY_ANSWERED_QUESTION_REPLY_VARIATIONS = {
    "I already told you - {subject} gets {count_str} \xF0\x9F\x92\xA5",
    "We've been through this - {subject} is {count_str} \xF0\x9F\x92\xA5",
    "As I said before, {subject} deserves {count_str} \xF0\x9F\x92\xA5",
    "Still {count_str} \xF0\x9F\x92\xA5 for {subject}, just like last time",
    "My answer for {subject} hasn't changed: {count_str} \xF0\x9F\x92\xA5",
    "Did you forget? {subject} is {count_str} \xF0\x9F\x92\xA5",
    "Pay attention! I said {subject} gets {count_str} \xF0\x9F\x92\xA5",
    "Let me repeat: {subject} deserves {count_str} \xF0\x9F\x92\xA5",
};

inline const std::vector<std::string> SASSY_REPLIES_WHAT = {
    "How many uhhhh?",
...
inline const std::vector<std::string> VICTORY_REASONS = {
...
inline const std::vector<std::string> BATTLE_OUTCOMES = {
...
inline const std::vector<std::string> CLOSE_MATCH_OUTCOMES = {
...
// Random pick (Python random.choice semantics).
inline const std::string& random_choice(const std::vector<std::string>& options) {
    static thread_local std::mt19937 rng{std::random_device{}()};
    return options[static_cast<size_t>(rng() % options.size())];
}

// Replace every occurrence of `key` in `tmpl` with `value`.
inline std::string format_reply(const std::string& tmpl,
    ...
    for (const auto& [key, value] : fields) {
        ...
        while ((at = out.find(from, at)) != std::string::npos) {
            out.replace(at, from.size(), value);
            at += value.size();
        }
    }
    return out;
...
inline std::string format_subject_reply(const std::string& tmpl, const std::string& subject,
                                        const std::string& count_str) {
    return format_reply(tmpl, {{"subject", subject}, {"count_str", count_str}});
}

inline std::string format_battle_reply(const std::string& tmpl, const std::string& winner,
                                       const std::string& loser, const std::string& reason) {
    return format_reply(tmpl, {{"winner", winner}, {"loser", loser}, {"reason", reason}});
}

// "💥" repeated `count` times ("count_str" in the Python handlers).
inline std::string booms_for(int count) {
    std::string out;
